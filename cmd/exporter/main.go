...
import (
	...
	handler "git.cplus.link/crema/backend/internal/services/exporter"
...
func main() {
	...
	serviceConf, err := configer.Service(iface.ExporterServiceName)
	if err != nil {
		panic(err)
	}

	service, err := handler.NewExporterService(configer)
	if err != nil {
		panic(err)
	}
