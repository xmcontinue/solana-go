...
type SwapTransaction struct {
	...
	TxData        string          `json:"tx_data"       gorm:"text(0)" `                            // 原数据（json格式）
}
...
type UserSwapCount struct {
	...
	UpdatedAt             *time.Time      `json:"-" gorm:"not null;type:timestamp(6);index"`
	LastSwapTransactionID int64           `json:"last_swap_transaction_id" gorm:"not null;default:0"`                                                               // 最后同步的transaction id
	UserAddress           string          `json:"user_address" gorm:"not null;type:varchar(64);  uniqueIndex:user_swap_count_user_address_swap_address_unique_key"` // 用户 address
	SwapAddress           string          `json:"swap_address" gorm:"not null;type:varchar(64);  uniqueIndex:user_swap_count_user_address_swap_address_unique_key"` // swap地址
	TokenAAddress         string          `json:"token_a_address" gorm:"not null;type:varchar(64);  index"`                                                         // swap token a 地址
	TokenBAddress         string          `json:"token_b_address" gorm:"not null;type:varchar(64);  index"`                                                         // swap token b 地址
	...
	UserTokenABalance     decimal.Decimal `json:"user_token_a_balance" gorm:"type:decimal(36,18);default:0"`                                                        // user swap  token a 余额
	UserTokenBBalance     decimal.Decimal `json:"user_token_b_balance" gorm:"type:decimal(36,18);default:0"`                                                        // user swap  token b 余额
	TxNum                 int64           `json:"tx_num"`                                                                                                           // 交易笔数
}
...
type UserSwapCountDay struct {
	...
	UpdatedAt             *time.Time      `json:"-" gorm:"not null;type:timestamp(6);index"`
	LastSwapTransactionID int64           `json:"last_swap_transaction_id" gorm:"not null;default:0"`                                                                        // 最后同步的transaction id
	UserAddress           string          `json:"user_address" gorm:"not null;type:varchar(64);  uniqueIndex:user_swap_count_day_user_address_swap_address_date_unique_key"` // 用户 address
	SwapAddress           string          `json:"swap_address" gorm:"not null;type:varchar(64);  uniqueIndex:user_swap_count_day_user_address_swap_address_date_unique_key"` // swap地址
	TokenAAddress         string          `json:"token_a_address" gorm:"not null;type:varchar(64);  index"`                                                                  // swap token a 地址
	TokenBAddress         string          `json:"token_b_address" gorm:"not null;type:varchar(64);  index"`                                                                  // swap token b 地址
	UserTokenAVolume      decimal.Decimal `json:"user_token_a_volume" gorm:"type:decimal(36,18);default:0"`                                                                  // swap token a 总交易额
	UserTokenBVolume      decimal.Decimal `json:"user_token_b_volume" gorm:"type:decimal(36,18);default:0"`                                                                  // swap token b 总交易额
	...
	Date                  string          `json:"date" gorm:"not null;type:varchar(64);uniqueIndex:user_swap_count_day_user_address_swap_address_date_unique_key"`           // 统计日期
}
