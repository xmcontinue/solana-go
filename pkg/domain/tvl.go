...
type SwapCountKLine struct {
	...
	TokenABalance         decimal.Decimal `json:"token_a_balance" gorm:"type:decimal(36,18);default:0"`                                                              // swap token a 余额
	TokenBBalance         decimal.Decimal `json:"token_b_balance" gorm:"type:decimal(36,18);default:0"`                                                              // swap token b 余额
	TokenASymbol          string          `json:"token_a_symbol"      gorm:"not null;type:varchar(64);  index:swap_count_k_line_token_a_symbol_date_type"`           // token A 币种符号
	TokenBSymbol          string          `json:"token_b_symbol"      gorm:"not null;type:varchar(64);  index:swap_count_k_line_token_b_symbol_date_type"`
	DateType              DateType        `json:"date_type"  gorm:"not null;type:varchar(64);  uniqueIndex:swap_count_k_line_date_swap_address_unique_key; index:swap_count_k_line_token_a_symbol_date_type;index:swap_count_k_line_token_b_symbol_date_type"` // 时间类型（min,quarter,hour,day,wek,mon）
