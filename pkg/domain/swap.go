...
type UserCountKLine struct {
	...
	CreatedAt                     *time.Time      `json:"-" gorm:"not null;type:timestamp(6);index"`
	UpdatedAt                     *time.Time      `json:"-" gorm:"not null;type:timestamp(6);index"`
	LastSwapTransactionID         int64           `json:"last_swap_transaction_id" gorm:"not null;default:0;index:idx_user_count_k_lines_last_swap_transaction_id_date_type"`                                                     // 最后同步的transaction id
	...
	DateType                      DateType        `json:"date_type" gorm:"not null;type:varchar(64);uniqueIndex:user_swap_tvl_count_day_swap_address_unique_key;index:idx_user_count_k_lines_last_swap_transaction_id_date_type"` // 时间类型（day,wek,mon）
