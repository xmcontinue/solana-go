...
type SwapTokenPriceKLine struct {
	...
	CreatedAt *time.Time      `json:"-" gorm:"not null;type:timestamp(6);index"`
	UpdatedAt *time.Time      `json:"-" gorm:"not null;type:timestamp(6);index"`
	Symbol    string          `json:"symbol" gorm:"not null;type:varchar(64);uniqueIndex:swap_token_price_k_line_symbol_date_date_type_unique_key; Index: SwapTokenPriceKLine_symbol_date_date_type_index"`    // symbol
	Open      decimal.Decimal `json:"open" gorm:"type:decimal(36,18);default:0"`                                                                                                                               // 统计时间段第一个值
	High      decimal.Decimal `json:"high" gorm:"type:decimal(36,18);default:0"`                                                                                                                               // 最大值
	Low       decimal.Decimal `json:"low"  gorm:"type:decimal(36,18);default:0"`                                                                                                                               // 最小值
	Settle    decimal.Decimal `json:"settle" gorm:"type:decimal(36,18);default:0"`                                                                                                                             // 结束值
	Avg       decimal.Decimal `json:"avg" gorm:"type:decimal(36,18);default:0"`                                                                                                                                // 平均值
	Num       int64           `json:"num"`                                                                                                                                                                     // 获取次数
	Date      *time.Time      `json:"date" gorm:"not null;type:timestamp(6);uniqueIndex:swap_token_price_k_line_symbol_date_date_type_unique_key; Index: SwapTokenPriceKLine_symbol_date_date_type_index"`     // 统计日期
	DateType  DateType        `json:"date_type" gorm:"not null;type:varchar(64);uniqueIndex:swap_token_price_k_line_symbol_date_date_type_unique_key; Index: SwapTokenPriceKLine_symbol_date_date_type_index"` // 时间类型（min,quarter,hour,day,wek,mon）
}
