...
func syncPrice(swapAccount string, t time.Time) (decimal.Decimal, decimal.Decimal, decimal.Decimal, decimal.Decimal, error) {
	...
	for {
		transactions, err := model.QuerySwapTransactionsV2(context.Background(), 100, 0, model.SwapAddressFilter(swapAccount), model.NewFilter("block_time > ?", t), model.OrderFilter("id asc"))
		if err != nil {
