...
func SwapTotalCount() error {
	...
	for _, v := range sol.SwapConfigList() {
		...
		if err != nil || newContractPrice.Settle.IsZero() || beforeContractPrice.Settle.IsZero() {
			continue
		}
...
// SwapTotalCountNew TODO 汇总统计(新版统计)
func SwapTotalCountNew() error {
	// 将要统计的数据
	swapCountToApi := &domain.SwapCountToApi{
		Pools:  make([]*domain.SwapCountToApiPool, 0),
		Tokens: make([]*domain.SwapCountToApiToken, 0),
	...
	for _, v := range sol.SwapConfigList() {
		...
		// 获取24h交易额，交易笔数 不做错误处理，有可能无交易
		swapCount24h, _ := model.SumSwapCountVolForKLinesNew(ctx, model.NewFilter("date > ?", before24hDate), model.SwapAddress(v.SwapAccount), model.NewFilter("date_type = ?", "1min"))

		// 获取过去7天交易额，交易笔数 不做错误处理，有可能无交易
		swapCount7d, _ := model.SumSwapCountVolForKLinesNew(ctx, model.NewFilter("date > ?", before7dDate), model.SwapAddress(v.SwapAccount), model.NewFilter("date_type = ?", "1min"))

		// 获取总交易额，交易笔数 不做错误处理，有可能无交易
		swapCountTotal, _ := model.SumSwapCountVolForKLinesNew(ctx, model.SwapAddress(v.SwapAccount), model.NewFilter("date_type = ?", "1min"))
	...
	swapCountKey := domain.SwapTotalCountKeyNew()
	if err := redisClient.Set(context.Background(), swapCountKey.Key, data, swapCountKey.Timeout).Err(); err != nil {
		return errors.Wrap(err)
	}
