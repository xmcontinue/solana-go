...
func Init(viperConf *config.Config) error {
	...
	err = conf.UnmarshalKey("api_host", &job.CremaHost)
	if err != nil {
		return errors.Wrap(err)
	}
	...
	_, err = job.Cron.AddFunc(getSpec("swap_count_cache"), WatchCremaHttpStatus)
	if err != nil {
		panic(err)
	}

	_, err = job.Cron.AddFunc(getSpec("swap_count_cache"), WatchExchangeSyncTime)
	if err != nil {
		panic(err)
	}
